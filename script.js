...
const isPositionOccupied = (x, y) => {
    return snakeBody.some(part => part[0] === x && part[1] === y) ||
...
const addObstacles = () => {
    ...
    for (let i = 0; i < numObstacles; i++) {
        ...
        } while (isPositionOccupied(obstacleX, obstacleY) || (Math.abs(obstacleX - snakeX) <= 1 && Math.abs(obstacleY - snakeY) <= 1));
...
const initGame = currentTime => {
    ...
    if (!specialItemExists && score == 20) { // Example: add special item if score is 15 or more
